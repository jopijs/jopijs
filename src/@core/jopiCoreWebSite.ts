...
import { testRoutePath, type RouteSelector, type WebSiteRouteInfos, type RouteHandler, type JopiWsRouteHandler, type JopiErrorHandler, type JopiRouteHandler, type HttpMethod } from "./routes.ts";
export { type HttpMethod } from "./routes.ts";
...
export class CoreWebSite {
     ...
     */
    async tryAuthUser<T = any>(loginInfo: T): Promise<AuthResult> {
        if (this.authHandler) {
...
export type FetchBodyAccepted = ReadableStream<Uint8Array> | string | FormData | null;
